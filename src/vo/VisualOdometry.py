...
from src.vo.FeaturesTracker import FeatureTrackerTypes, FeatureTrackingResult
from src.utils.geometric import poseRt
from src.timer import TimerFps
...
class VisualOdometry(object):
    ...
    def getAbsoluteScale(self, frame_id):  
        if self.groundtruth is not None and kUseGroundTruthScale:
            temp = self.groundtruth._getpose(frame_id)
            print(temp)
            self.trueX, self.trueY, self.trueZ, scale = temp
            return scale
        else:
            self.trueX = 0 
            self.trueY = 0 
    ...
    def estimatePose(self, kps_ref, kps_cur):	
        ...
        if kUseEssentialMatrixEstimation:
            ...
            E, self.mask_match = cv2.findEssentialMat(self.kpn_cur, self.kpn_ref, focal=focal, pp=pp, method=cv2.RANSAC, prob=kRansacProb, threshold=kRansacThresholdNormalized)
        ...
        _, R, t, mask = cv2.recoverPose(E, self.kpn_cur, self.kpn_ref, focal=focal, pp=pp)   
    ...
    def updateHistory(self):
        ...
        if (self.t0_est is not None) and (self.t0_gt is not None):             
            ...
            p = [self.cur_t[0] , self.cur_t[1] , self.cur_t[2]-self.t0_est[2]]   # the estimated traj starts at 0
